...
assert.equal(subscriptionEntitlements.plus.maxCameraQuality, "high");
assert.equal(subscriptionEntitlements.plus.maxScreenShareQuality, "high");
assert.equal(subscriptionEntitlements.plus.maxAttachmentBytes, 20 * 1024 * 1024);
assert.equal(subscriptionEntitlements.plus.animatedProfile, false);
assert.equal(subscriptionEntitlements.pro.maxAttachmentBytes, 100 * 1024 * 1024);
assert.equal(subscriptionEntitlements.pro.animatedProfile, true);
assert.deepEqual(subscriptionEntitlements.ultimate, subscriptionEntitlements.pro);
assert.deepEqual(subscriptionEntitlements.max_supporter, subscriptionEntitlements.pro);
assert.equal(isPaidSubscription("plus"), true);
assert.equal(isPaidSubscription("pro"), true);
assert.equal(isPaidSubscription("ultimate"), true);
assert.equal(isPaidSubscription("max_supporter"), true);
