...
export function usageAmount(provider: RtcProviderId, seconds: number) {
  ...
  if (["agora", "tencent", "100ms", "cometchat", "whereby", "livekit"].includes(provider)) return billedMinutes;
  ...
  // unique MAU and VideoSDK by provider-reported credit, so client seconds are
  // not converted to those units. Legacy Daily has no managed quota policy.
  return null;
}
